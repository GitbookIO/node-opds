...
var addBookEntry = function(feed, book) {
	...
	if (book.issued) entry.ele("dc:issued").txt(""+book.issued.getFullYear()+"-"+(book.issued.getMonth()+1)+"-"+book.issued.getDate());
	if (book.publisher) entry.ele("dc:publisher").txt(book.publisher);
	...
	if (book.content) {
		var content = _.isString(book.content)? { type: "text", content: book.content} : book.content;
		var cel = entry.ele("content", { type: content.type });
		if (content.type == "text") {
			cel.txt(content.content);
		} else {
			cel.raw(content.content);
		}
	}
	if (book.language) entry.ele("dc:language").txt(book.language);
